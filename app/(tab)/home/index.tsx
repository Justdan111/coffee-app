...
const COFFEE_DATA = [
  ...
  },
  {
    ...
    rating: 4.8,
    reviews: 230,
    image: "https://images.unsplash.com/photo-1509785307050-d4066910ec1e?w=500&h=500&fit=crop",
  },
]
...
export default function HomeScreen() {
  ...
  const CoffeeCard = ({ item, index }: { item: (typeof COFFEE_DATA)[0]; index: number }) => (
    <AnimatedView entering={FadeInUp.delay(index * 100).duration(500)} className="flex-1 m-2">
      <Pressable onPress={() => navigateToDetail(item)} className="coffee-card active:opacity-70">
        <Image source={{ uri: item.image }} style={{ width: "100%", height: 180, borderRadius: 12, marginBottom: 8 }} />
  ...
  return (
    <SafeAreaView className="flex-1 bg-secondary">
      <AnimatedView entering={FadeInDown.duration(500)} className="px-5 pt-6 pb-4  rounded-xl p-4">
        <View className="flex-row justify-between items-center mb-6 ">
          <View>
            ...
            <View className="flex-row items-center mt-1">
              ...
              <Text className="text-white font-semibold ml-2">Bilzen, Tanjungbalai</Text>
            </View>
          </View>
        ...
        <View className="bg-gradient-to-r from-primary to-accent rounded-2xl p-6 mb-6 ">
          <Text className="text-white bg-red-500 px-3 py-1 rounded-full inline-block mb-2 w-16 text-center font-semibold text-sm">
            Promo
          </Text>
